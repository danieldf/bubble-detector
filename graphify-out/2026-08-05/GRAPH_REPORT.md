...
## Corpus Check
- 38 files · ~88,615 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 270 nodes · 467 edges · 18 communities (17 shown, 1 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 32 edges (avg confidence: 0.77)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `29e71e7a`
...
1. `DataIngestor` - 24 edges
2. `DashboardState` - 22 edges
3. `StructuralBreakPredictor` - 18 edges
4. `compute_tda_wavelet_complexity()` - 14 edges
5. `compute_gsadf_gpt_decomposition()` - 13 edges
6. `compute_margin_leverage_metrics()` - 12 edges
7. `compute_macro_valuations()` - 12 edges
8. `compute_options_volatility_metrics()` - 12 edges
9. `Market Bubble Structural Analysis Report` - 12 edges
10. `compute_technical_indicators()` - 11 edges
...
Cohesion: 0.44
Nodes (10): build_econometric_fig(), build_leverage_fig(), build_macro_valuation_fig(), build_sector_health_fig(), build_sentiment_vol_fig(), generate_explanatory_markdown(), DataFrame, Figure (+2 more)
...
- **Are the 4 inferred relationships involving `DataIngestor` (e.g. with `DataFetchError` and `ValidationError`) actually correct?**
  _`DataIngestor` has 4 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 3 inferred relationships involving `StructuralBreakPredictor` (e.g. with `ModelTrainingError` and `DashboardState`) actually correct?**
  _`StructuralBreakPredictor` has 3 INFERRED edges - model-reasoned connections that need verification._
