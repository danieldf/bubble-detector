...
def postprocess_wasm_html(html_path: Path = Path("dist/index.html")):
    ...
    # 4. Inject Progressive Checklist Styles & DOM Overlay (idempotently)
    if "wasm-progressive-boot-style" not in content:
        styles = """<style id="wasm-progressive-boot-style">
  #wasm-splash-overlay { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: #0F172A; color: #F8FAFC; z-index: 999999; display: flex; flex-direction: column; align-items: center; justify-content: center; font-family: -apple-system, BlinkMacSystemFont, "SF Pro Text", Inter, sans-serif; transition: opacity 0.4s ease; }
  .splash-box { background: #1E293B; border: 1px solid #334155; border-radius: 16px; padding: 2.5rem 3rem; max-width: 520px; width: 90%; box-shadow: 0 20px 40px rgba(0,0,0,0.5); text-align: center; }
  .splash-title { font-size: 1.4rem; font-weight: 700; margin-bottom: 0.5rem; color: #FFFFFF; display: flex; align-items: center; justify-content: center; gap: 0.5rem; }
  .splash-subtitle { font-size: 0.88rem; color: #94A3B8; margin-bottom: 1.8rem; }
  .checklist { list-style: none; padding: 0; margin: 0 0 1.5rem 0; text-align: left; font-size: 0.95rem; }
  .checklist-item { display: flex; align-items: center; gap: 0.75rem; padding: 0.6rem 0; border-bottom: 1px solid #334155; color: #64748B; transition: color 0.3s ease; }
  .checklist-item:last-child { border-bottom: none; }
  .checklist-item.active { color: #38BDF8; font-weight: 600; }
  .checklist-item.done { color: #34D399; }
  .checklist-icon { font-size: 1.1rem; width: 1.5rem; text-align: center; }
...
<div id="wasm-splash-overlay" role="dialog" aria-modal="true" aria-label="Application Loading">
  <div class="splash-box">
    <div class="splash-title">🏛️ Market Bubble Detector</div>
    <div class="splash-subtitle">WebAssembly Edition • Institutional Risk Analytics</div>
...
function initHashRouting() {
  function parseHash() {
    ...
    return { tab: params.get('tab'), horizon: params.get('horizon') };
  }
  function syncHashState() {
    const state = parseHash();
    if (state.tab) {
      const tabButtons = document.querySelectorAll('.bk-tab, [role="tab"], .tab');
      tabButtons.forEach(btn => {
        if (btn.textContent && btn.textContent.toLowerCase().includes(state.tab.toLowerCase())) {
          btn.click();
        }
      });
    }
...
if (document.readyState === 'loading') {
  ...
  initKeyboardAccessibility();
    ...
    if "updateProgressChecklist('core'" not in content:
        # Normalize old preloader block if present from previous runs
        old_preloader = r"for\s*\(\s*const\s*ds\s*of\s*\[\s*'market_data_50yr\.json'.*?document\.body\.prepend\(el\);\s*\}"
        content = re.sub(old_preloader, "await pyodide.runPythonAsync(code);", content, flags=re.DOTALL)

        # Wrap main() body with try-catch and checklist tracking
      ...
      try {
        ...
        if (typeof handleResponsiveLegends === 'function') handleResponsiveLegends();
